...
class PtDetector(object):
  ...
  def make_prototypes(self, features):
    """
    input: list of tuples (feature, label)
    output: 
    """
    support_features = torch.cat([item[0] for item in features])
  ...
  def threshold_calc(self, distances, known_labels, std_coefficient=1.0):
