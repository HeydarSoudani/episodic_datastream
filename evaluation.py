...
def final_step_evaluation(results, base_labels, known_labels, k=(1, 5,), eps=1e-8):
  ...
  ow_results = results
  ow_results[np.isin(ow_results['predicted_label'], list(base_labels), invert=True)]['predicted_label'] = -1
  ow_results[np.isin(ow_results['true_label'], list(base_labels), invert=True)]['true_label'] = -1
  OwCA = accuracy_score(
...
def evaluate(results, known_labels, k=(1, 5,), eps=1e-8):
  ...
  return M_new, F_new, CwCA, OwCA, NCA, cm
